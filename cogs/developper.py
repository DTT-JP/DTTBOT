...
# .envからアクセスURLを取得
DEVELOPPER_UUID = os.getenv("DEVELOPPER_UUID")
...
def is_developer():
    def predicate(interaction: discord.Interaction):
        ...
        if not DEVELOPPER_UUID:
            return False
        # 実行者のIDがDEVELOPPER_UUIDと一致すればTrue
        return str(interaction.user.id) == DEVELOPPER_UUID
...
async def load_extension_command(self, interaction: discord.Interaction, cog_name: str):
    ...
    # すぐに応答しないとタイムアウトするため、deferで応答を保留します
    ...
    try:
        ...
        # 既にロードされている場合
        await interaction.followup.send(f'⚠️ **WARNING**：`{cog_name}` は既にロードされています。', ephemeral=True)
...
async def unload_extension_command(self, interaction: discord.Interaction, cog_name: str):
    ...
    # 自身のコグをアンロードしようとした場合のエラー処理
    if cog_name == 'cogs.developer': # cogs/developer.py のドットパスを指定
         await interaction.followup.send(f'❌ **ERROR**：自身のコグ (`{cog_name}`) はアンロードできません。', ephemeral=True)
         return
    ...
    try:
        ...
        await interaction.followup.send(f'✅ **SUCCESS**：`{cog_name}` をアンロードしました。', ephemeral=True)
         ...
         # 既にアンロードされている場合
             await interaction.followup.send(f'⚠️ **WARNING**：`{cog_name}` は既にアンロードされています。', ephemeral=True)
    except commands.ExtensionNotFound:
        # ファイルが見つからない場合 (アンロードでも発生しうる)
        await interaction.followup.send(f'❌ **ERROR**：`{cog_name}` という名前のCogが見つかりませんでした。', ephemeral=True)
    except Exception as e:
        # その他のエラー
